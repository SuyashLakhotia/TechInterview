...
Generally, Dijkstra's algorithm takes `O(|E| * T_dk + |V| * T_em)` where `T_dk` & `T_em` denote the complexities of the *decrease-key* and *extract-minimum* operation (both are `O(log |V|)` for a binary heap).
